...
/// \brief Returns the name of the event's node.
///
/// \return The node name
QString QXmppPubSubEvent::node() const
{
    return d->node;
}

/// \brief Sets the name of the event's node.
///
/// \param node
void QXmppPubSubEvent::setNode(const QString &node)
{
    d->node = node;
}

/// \brief Returns the PubSub items of the event.
///
/// \return A list of PubSub items.
QList<QXmppPubSubItem> QXmppPubSubEvent::items() const
{
    return d->items;
}

/// \brief Sets the PubSub items of the event.
///
/// \param items
...
void QXmppPubSubEvent::parse(const QDomElement &element)
{
    ...
    d->node = itemsElement.attribute("node");
